...
> провайдера). Слой B — **база знаний** `src/knowledge-base` (только тексты, без дат): RU-канон в
> корне папки, полные переводы в `uk/` и `en/` с теми же именами файлов, схемой и `id`. В кодовой базе **нет захардкоженных дат/годов**: приложение
...
> Языки: KB переведена на RU/UK/EN; тексты берутся из KB активного языка, фиксированные подписи
> (фазы, знаки, «Луна переходит в знак…», типы затмений) — из i18n (ключи `daily:kb.*`). Баннер
> «перевод недоступен» показывается только если в выбранном языке нет записи и взят RU (§7.5).
...
5. **Нет записи в языке.** Все три языка KB переведены полностью, поэтому в штатном режиме баннер не появляется. Если в `uk/` или `en/` не хватает файла или записи (например, добавили новый лунный день только в RU), берётся RU-запись по тому же `id` и показывается пометка «перевод недоступен» вместо пустоты. Значения-идентификаторы (`status`, ключи знаков в `eclipses.json`) всегда читаются из RU.
